...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/articles?filters[category][slug][$containsi]=profil&filters[publication_date][$lte]=${encodeURIComponent(now)}&pagination[pageSize]=10&sort=publication_date:desc&populate=*&status=published`)
      .then(async (r) => {
          ...
          <div className="lg:col-span-9 space-y-12 md:space-y-16">
            ...
            {!loading && sorted.length === 0 && (
              <div className="bg-surface-white border border-border-light rounded-xl p-8 text-center">
                ...
                <Link href="/berita?category=profil" className="inline-flex mt-4 bg-primary text-on-primary px-6 py-2 rounded-lg">Lihat Berita Kategori Profil</Link>
              </div>
            )}
