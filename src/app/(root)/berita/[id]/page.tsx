...
export default function ArticleDetailPage() {
  ...
  useEffect(() => {
    ...
    const filterField = isDocId ? 'documentId' : 'slug';
    const articleQuery = `/api/articles?filters[${filterField}][$eq]=${encodeURIComponent(idStr)}&populate[category][fields][0]=name&populate[category][fields][1]=slug&populate[tags][fields][0]=name&populate[tags][fields][1]=slug&populate[author][fields][0]=firstname&populate[author][fields][1]=lastname&populate[featuredImage][populate]=*&populate[relatedArticles][populate][articles][populate][featuredImage][populate]=*&populate[relatedArticles][populate][articles][populate][category][fields][0]=name&populate[relatedArticles][populate][articles][populate][category][fields][1]=slug&populate[pdfViewer][populate]=*&status=published`;
    ...
    fetch(articleQuery)
      ...
      .then((j) => {
        ...
        if (!found) {
          // Fallback: if slug failed but looks like maybe try other field (shouldn't happen often)
          if (!isDocId) {
            // Try documentId as fallback if slug not found
            return fetch(`/api/articles?filters[documentId][$eq]=${encodeURIComponent(idStr)}&populate=*&status=published`)
              .then((r2) => r2.json())
              .then((j2) => {
                if (cancelled) return;
                const found2 = Array.isArray(j2.data) ? j2.data[0] : j2.data;
                if (!found2) {
                  setError('Artikel tidak ditemukan');
                  setLoading(false);
                } else {
                  setArticle(found2 as ArticleCMS);
                  setLoading(false);
                }
              })
              .catch((e2) => {
