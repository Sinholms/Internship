...
export default function UnduhanPage() {
  const { articles: downloadArticles, loading } = useArticles({ category: 'download', pageSize: 20 });
          ...
          {docs.map((doc, i) => (
            <div key={i} className="bg-surface-white rounded-xl border border-border-light shadow-sm p-4 md:p-6 flex flex-col sm:flex-row sm:items-center gap-4 hover:shadow-md transition-shadow">
              ...
              {(doc as any).href && (doc as any).href !== '#' ? (
                <Link to={(doc as any).href} className="inline-flex items-center gap-2 bg-primary text-on-primary px-5 py-2.5 rounded-lg font-label-md text-label-md font-bold hover:opacity-90 transition-opacity shrink-0">
