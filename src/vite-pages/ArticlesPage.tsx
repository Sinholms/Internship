...
export default function ArticlesPage() {
  ...
  const { articles, loading, error, pageCount } = useArticles({
    query,
    category: activeCat === 'Semua' ? undefined : activeCat,
    page: pageParam,
    pageSize: 9,
    sort: 'desc',
  });
      ...
      <section className="py-10 md:py-section-padding max-w-container-max mx-auto px-4 md:px-margin-desktop">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 mb-8 md:mb-10">
          <div>
            ...
            <p className="text-body-md font-body-md text-on-surface-variant mt-2">Pilih kategori untuk menemukan informasi yang Anda butuhkan. {error ? '(Fallback aktif)' : ''}</p>
          </div>
          <div className="flex flex-wrap gap-2" aria-label="Filter kategori berita">
            {filterCategories.slice(0, 6).map((cat: string) => (
              <button
                key={cat}
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-gutter">
              {articles.map(article => {
                ...
                return (
                  <article key={article.documentId} className="bg-surface-white rounded-xl overflow-hidden border border-border-light hover-card flex flex-col">
                    ...
                    <div className="p-5 md:p-6 flex flex-col grow">
                      ...
                      <Link className="mt-5 text-primary font-label-md text-label-md font-bold flex items-center gap-2 hover:underline" to={`/berita/${article.slug}`}>Baca Selengkapnya <span className="material-symbols-outlined text-sm">arrow_forward</span></Link>
                    </div>
                  </article>
                );
              })}
